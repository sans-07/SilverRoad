# Silver Road - 이용자 위치 알림 서비스

'이용자'의 위치를 '보호자'가 확인하고, 지정된 '세이프존' 이탈 시 알림을 받을 수 있는 웹 애플리케이션입니다.

## 주요 기능

-   **역할 기반 사용자 등록**: '보호자'와 '이용자' 역할 선택 가능
-   **연결 관리**: 이용자가 보호자에게 연결을 요청하고, 보호자가 수락하여 계정을 연동
-   **실시간 위치 추적**: 이용자의 위치를 주기적으로 수집하여 지도에 경로 표시
-   **자동 세이프존 생성**: 자주 방문하는 위치를 기반으로 '세이프존' 자동 계산 및 시각화
-   **이탈 알림**: 이용자가 세이프존을 벗어날 경우, 보호자에게 실시간 알림 전송
-   **보호자 대시보드**: 연결 요청, 연결된 이용자 목록, 이탈 알림 등을 한눈에 확인
...
`temp_data_script.txt` 파일의 스크립트를 사용하면 '세이프존' 생성 및 이탈 알림 기능을 테스트하기 위한 더미 위치 데이터를 쉽게 추가할 수 있습니다.

1.  '이용자' 역할의 사용자로 로그인하여 메인 페이지로 이동합니다.
